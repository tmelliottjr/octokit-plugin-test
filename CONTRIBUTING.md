...
We appreciate you taking the time to contribute to `octokit-plugin-test`. Especially as a new contributor, you have a valuable perspective that we lost a long time ago: you will find things confusing and run into problems that no longer occur to us. Please share them with us, so we can make the experience for future contributors the best it could be.
...
1. Please make sure there is no [open issue](https://github.com/tmelliottjr/octokit-plugin-test/issues?utf8=%E2%9C%93&q=is%3Aissue) yet.
...
```shell
git clone https://github.com/<your account name>/octokit-plugin-test.git
cd octokit-plugin-test
npm install
```
...
- Submit a pull request from your topic branch to the main branch on the `tmelliottjr/octokit-plugin-test` repository.
- Be sure to tag any issues your pull request is taking care of / contributing to. Adding "Closes #123" to a pull request description will automatically close the issue once the pull request is merged in.

## Testing a pull request from github repo locally:

You can install `octokit-plugin-test` from each pull request. Replace `[PULL REQUEST NUMBER]`:

```
npm install https://github.pika.dev/tmelliottjr/octokit-plugin-test/pr/[PULL REQUEST NUMBER]
```

Once you are done testing, you can revert back to the default module `octokit-plugin-test` from npm with `npm install octokit-plugin-test`
